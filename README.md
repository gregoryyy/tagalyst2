...
- **Extension Options** – open `chrome://extensions`, click **Tagalyst 2 → Details → Extension options**, and use the **Enable** column in the Features table to turn the Search and Tag panes on/off (the “Expands” column is a placeholder). Disabling a pane hides it and clears its state. The same page also shows current storage usage and offers a “Delete” button if you need a full reset.
