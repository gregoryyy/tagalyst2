...
- **Extension Options** – open `chrome://extensions`, click **Tagalyst 2 → Details → Extension options**, and use the checkboxes to control both **visibility** and **interactivity** for Search and Tag filtering. Hiding a feature removes the panel entirely; turning off interactivity keeps the panel visible but grays it out and clears its state. The same page also shows current storage usage and offers a “Delete” button if you need a full reset.
